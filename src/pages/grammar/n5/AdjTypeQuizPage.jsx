...
export default function AdjTypeQuizPage() {
  ...
  const { level: rawLevel = "n5", lesson = "lesson1" } = useParams();
  ...
  return (
    <MCQQuizShell
      ...
      onExit={handleExit}
      renderQuestion={(q) => (
        <span className="jp" style={{ fontSize: 28 }}>
          {q?.question}
        </span>
      )}
      renderChoice={(c) => (
        <ChoiceFace label={c} lang={i18n.language.toLowerCase()} />
      )}
      // 2択なので番号ラベルは非表示
      numberLabels={[]}
    />
