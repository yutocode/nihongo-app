...
// === id から品詞を決定 ===
export function getPosById(level, id, fallback = "その他") {
  const ranges = POS_RANGE_BY_LEVEL[level];
...
export function getPosOrder(level) {
  const ranges = POS_RANGE_BY_LEVEL[level];
  if (ranges) return ranges.map((r) => r.pos);
  return ["名詞", "い形容詞", "な形容詞", "副詞", "動詞", "その他"];
}
